 ...
 *
 * Devil is the complex messager with distributed structure.
 * It has a server (devil itself) with several modules like "message protocol module", "log module"
 * "sync module", "UI module", etc...
 *
 * All sistem works via events mecanism. Modules subscribe for events and got them when such events
 * occour. For example UIModule and LogModule subscribed for event with type "New_Message_Income_Event", which
 * can be generated by MessageProtocolModule. This event contains information about message and is sent
 * to subscribers. The Request-Responce scheme is works via this mechanism.
 *
 * All system is highly-threaded. Each module runs in its own thread. And most of handlers calls in their
 * threads too.
 */

//TODO make events represents mosts of Devil methods.
//TODO start EventProcessor from Devil.main()
//TODO load modules with class path.
...
public class Devil {
     ...
     * It wil raise when EventManager gets it in enother thread.
     *
